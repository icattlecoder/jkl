...
func NewSite(src, dest, plugin string) (*Site, error) {

	plugins = append(plugins, new(FC))
	...
	mplugin := make(map[string][]string)
...
func (s *Site) Generate() error {
	...
	for _, p := range plugins {
		if v, ok := s.plugin[p.PlugeName()]; ok {
			p.SetArgs(v)
			p.OnSiteGenarated(s)
		}
	}
